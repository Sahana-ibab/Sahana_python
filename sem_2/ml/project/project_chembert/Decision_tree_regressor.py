...
# Function: Hyperparameter tuning for DecisionTreeRegressor model
#  - Parameters: X_train, y_train (training features and labels)
#  - Uses: GridSearchCV to perform 3-fold cross-validation
#          on a predefined grid of DecisionTreeRegressor parameters
#  - Returns: dictionary of -best parameters (max_depth, min_samples_split, min_samples_leaf)

def tune_hyperparameters(X_train, y_train):
    param_grid = {
        'max_depth': [3, 6, 9],
        'min_samples_split': [2, 5, 10],
        'min_samples_leaf': [2, 5, 5]
    }
...
# Function: k-fold cross-validation with DecisionTreeRegressor and PCA
#  - Parameters:
...
#           - Trains DecisionTreeRegressor with -best parameters
...
def k_fold_cross_validation(df, n_splits=10):
    ...
    plt.title("DecisionTreeRegressor Regression Scatter (All Folds)")
    plt.legend()
    plt.show()
